...
%}
    ...
    while i < max_iter && IS_divergence(V, W*H) > done_thresh
        ...
        i = i + 1;
        %{
        if IS_divergence(V, W*H) > lastDiv
            % this is in an if so we can breakpoint it and examine the environment etc
            % hacking!
            assert(10 == 3, "update rules failed to monotonically decrease divergence!")
        end
        lastDiv = IS_divergence(V, W*H);
        %}
