...
%    @(s, si)block_normalise(align_onset_specDiff_rectL2(s,si),100), ...
%   @(s,si)align_onset_bandSplit(s,si,@align_onset_specDiff_taxi) 
%    @(s,si)align_onset_bandSplit(s,si,@align_onset_specDiff_rectL2)
%   @(s,si)row_normalise(align_onset_bandSplit(s,si,@align_onset_specDiff_taxi),100) ...
%    @(s,si)row_normalise(align_onset_bandSplit(s,si,@align_onset_specDiff_rectL2), 100), ...
};
...
for i = 1:length(test_funcs)
    ...
    for j = 1:length(audio_paths)       
        ...
        if WRITE_TMP
            ...
            write_to_file(onsets, audio, spectInfo, filename);
        end
        ...
        if WRITE_TMP_12D && ~iscolumn(onsets)
            ...
            for k = 1:12
                ...
                write_to_file(onsets(k, :).', audio, spectInfo, filename);
            end
        end
    end
end

function write_to_file (onsets, audio, spectInfo, filename)
