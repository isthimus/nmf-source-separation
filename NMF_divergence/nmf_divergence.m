function [W_out, H_out, iterations] = nmf_divergence (V, W, H, threshold, varargin)
%{    
non-negative matrix factorization algorithm - repeatedly updates W and H
using a pair of update rules until the matrix divergence between V
...
%}
    ...
    if isempty(W) || isempty(V) || isempty(H)
        ME = MException ("nmf_divergence:bad_input", "one or more of V, W, H is empty");
        throw(ME)
    end
    
    % check threshold is sensible
    if threshold < 0
        ME = MException ("nmf_divergence:bad_input", "threshold must be non negative");
        throw (ME)
    end
    
    % check for negative values in input
    if ~isempty(V(V < 0)) || ~isempty(W(W < 0)) || ~isempty(H(H < 0))
        ME = MException ("nmf_divergence:bad_input", "W,V,H must have all elements >= 0");
        throw(ME)
    end
    
    % check matrix dimensions
    if size(V)(1) ~= size(W)(1) || size(V)(2) ~= size(H)(2)
        ME = MException ("nmf_divergence:bad_input", "W * H must have the same shape as V");
        throw (ME)
    end
    ...
    if matrix_divergence (V, W*H) > threshold
        ME = MException ("nmf_divergence:failed_to_converge", "hit max iterations and still not within threshold");
        throw(ME)
    end
